...
export default function CoveragePage() {
  ...
  const [selectedCountry, setSelectedCountry] = useState<typeof coveredCountries[0] | null>(null)

  const filteredCountries = coveredCountries.filter(country => {
  ...
  const flyToCountry = (country: typeof coveredCountries[0]) => {
    setSelectedCountry(country)
    map.current?.flyTo({
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">
                            {String.fromCodePoint(...[...country.code].map(c => 0x1F1E6 - 65 + c.charCodeAt(0)))}
                          </span>
